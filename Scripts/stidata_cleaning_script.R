...
df1 <- df %>%
  ...
  dplyr::mutate (
    Sexdif = ifelse(df$Sex...47 == df$Sex...35, "Same", "Different"),
    #check if exact match
    Sex = case_when(
      Sexdif %in% c("Same", "Different") ~ df$Sex...47,
      #reassign gender!is.na(df$Sex...47) &
      is.na(df$Sex...35) ~ df$Sex...47,
      !is.na(df$Sex...35) &
        is.na(df$Sex...47) ~ df$Sex...35,
...
df1 <- df1 %>%
  dplyr::mutate(IdNumber2 = case_when(df1$IdNumber == 51 &
                                        df1$A1Age == 23 ~ 227, TRUE ~ df1$IdNumber)) %>%
  select(-c(IdNumber)) %>%
...
df1 <- df1 %>%
  dplyr::mutate(
    CaseStatus2 = case_when(
      df1$CaseStatus == 3 & df1$IdNumber == 31 ~ 1,
      df1$CaseStatus == 3 &
        df1$IdNumber == 1 ~ 2,
      TRUE ~ df1$CaseStatus
    )
...
df1 <- df1 %>%
  mutate(
    AgeCat = case_when(
      ...
      A1Age >= 20 &
        A1Age <= 39 ~ "Young adults",
      A1Age >= 40 &
        A1Age <= 59 ~ "Middle-aged adults",
      A1Age >= 60 &
        A1Age <= 79 ~ "Older adults",
      A1Age >= 80  ~ "Elderly",
...
df1 <- calculate_BMI(df1, Weight, Height)
...
df1 <- df1 %>%
  mutate(
    Typeofsti = case_when(
      CaseStatus == "Positive" & Typeofsti == "Gud" ~ "Gonorrhea",
      CaseStatus == "Positive" & Typeofsti == "Ud" ~ "Syphilis",
      CaseStatus == "Negative" & IdNumber =="1" ~ "HPV",
      TRUE ~ "None"
    )
  )
...
writexl::write_xlsx(df2,
                    "../Visualization_With_R/Visualizing_Gapminder/Data/STIData_Cleaned.xlsx")
